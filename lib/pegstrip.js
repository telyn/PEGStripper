		...
		switch(c) {
			case '{':
				if(inQuotes || inRegex) {
					...
					//! and & are conditionals which use javascript.
					// Strip the javascript but keep the !{} and &{} around
					// to indicate, "Hey, there's a conditional here"
			...
			case '/':
				if(!inComment && input[i+1] == '*') {
					...
					print(c); // still part of the comment
					inComment = false;
				} else {
					print(c);
				}
